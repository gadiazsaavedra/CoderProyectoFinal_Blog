...
def editarBlog(request, numero_para_editar):
    blog = Blog.objects.get(numero=numero_para)
    if request.method == 'POST':
        ...
        miFormulario = BlogFormulario(initial={'title': blog.title, 'description': blog.description, 'blogger': blog.blogger, 'created_at': blog.created_at, 'updated_at': blog.updated_at})
        
        
    return render(request, 'AppBlog/editarBlog.html', {'miFormulario': miFormulario})

def eliminarBlog(request):
    blogQueQuieroBorrar = Blog.objects.get()

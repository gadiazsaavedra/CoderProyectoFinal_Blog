...
def editarBlog(request, numero_para_editar):
    ...
    return render(request, 'AppFamiliar/editarBlog.html', {'miFormulario': miFormulario,"numero_para_editar": numero_para_editar})
...
def eliminarBlog(request, numero_para_borrar):
    ...
    return render(request, 'AppFamiliar/leerBlogs.html', {'Blogs': Blogs})

@login_required
def leerBlogs(request):
    Blogs = Blog.objects.all()
    dic = {"Blogs": Blogs}
    return render(request, "AppFamiliar/leerBlogs.html", dic)
